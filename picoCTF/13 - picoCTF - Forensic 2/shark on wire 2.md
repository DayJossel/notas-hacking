...
## Solución:
De nuevo, recibimos un archivo de captura:
...
Intentar la misma estrategia que antes no conduce a ningún resultado esta vez:
...
Inspeccionando los diferentes flujos UDP, podemos ver una secuencia interesante que comienza con un mensaje de "inicio" y termina con un mensaje de "fin":
...
Echemos un vistazo más de cerca a estos dos mensajes:
...
Los atributos similares son que ambos se enviaron a 10.0.0.1, el puerto de origen es 5000 y el puerto de destino es 22.

Filtrando según el puerto de origen y destino, solo obtenemos estos dos mensajes:
...
Sin embargo, filtrando solo según el puerto de destino, obtenemos una lista de mensajes entre el inicio y el final:
...
Podemos ver que el segundo mensaje se origina en el puerto de origen 5112. 112 es un número que debería alertarnos, ya que su representación ASCII es , que coincide con la plantilla de bandera.`p`

Por lo tanto, podemos intentar traducir los puertos de origen a ASCII usando el siguiente script:
